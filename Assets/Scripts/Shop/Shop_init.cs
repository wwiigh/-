...
public class Shop_init : MonoBehaviour
{
    ...
    void Add_Product(string name, string type,int price)
    {
        if(type == "item")
        {
            ...
            if(sell_list.Count<num_per_row) new_item.GetComponentInChildren<UI_Control>().reset_pos(this.transform,new Vector3(x,-3f,0));
            if(sell_list.Count>=num_per_row) new_item.GetComponentInChildren<UI_Control>().reset_pos(this.transform,new Vector3(x,-6f,0));
