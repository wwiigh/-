...
public class show_item : MonoBehaviour
{
    ...
    void OnEnable()
    {
        CardPos = CardObj.GetComponent<Transform>().position;
        ItemPos = ItemObj.GetComponent<Transform>().position;
        EquiPos = EquiObj.GetComponent<Transform>().position;
        RelicPos = RelicObj.GetComponent<Transform>().position;
        ItemWidth = ItemObj.GetComponent<RectTransform>().rect.width;
        ItemHight = ItemObj.GetComponent<RectTransform>().rect.height;
        CardPos = Camera.main.WorldToScreenPoint(CardPos);
        ItemPos = Camera.main.WorldToScreenPoint(ItemPos);
        EquiPos = Camera.main.WorldToScreenPoint(EquiPos);
        RelicPos = Camera.main.WorldToScreenPoint(RelicPos);
        CardPos = new Vector2(CardPos.x - ItemWidth/2, CardPos.y - ItemWidth/2);
        ItemPos = new Vector2(ItemPos.x - ItemWidth/2, ItemPos.y - ItemWidth/2);
        EquiPos = new Vector2(EquiPos.x - ItemWidth/2, EquiPos.y - ItemWidth/2);
        RelicPos = new Vector2(RelicPos.x - ItemWidth/2, RelicPos.y - ItemWidth/2);
    ...
    void Update()
    {
        ...
        if(WhereMouse > 0 && MousePos != LastMousePos){
            ...
            switch(WhereMouse){
                case 1:
                    card_name(get_card.id);
                    break;
                case 2:
                    item_name(get_items.id);
                    break;
                case 3:
                    equi_name(get_equi.id);       
                    break;
                case 4:
                    relic_name(get_relic.id);
                    break;
            }
    ...
    int WhereIsMouse(Vector2 Mouse){
        ...
        if(Mouse.x - ItemPos.x < ItemWidth && Mouse.x - ItemPos.x > 0 && Mouse.y - ItemPos.y < ItemHight && Mouse.y - ItemPos.y > 0){
            // Debug.Log("in item");
            return 2;
        }
        if(Mouse.x - EquiPos.x < ItemWidth && Mouse.x - EquiPos.x > 0 && Mouse.y - EquiPos.y < ItemHight && Mouse.y - EquiPos.y > 0){
            // Debug.Log("in equi");
            return 3;
        }
        if(Mouse.x - RelicPos.x < ItemWidth && Mouse.x - RelicPos.x > 0 && Mouse.y - RelicPos.y < ItemHight && Mouse.y - RelicPos.y > 0){
            // Debug.Log("in relic");
            return 4;
        }
        return 0;
    ...
    void card_name(int id){
        ...
        switch(id){
            ...
            case 16:
                ret = "還擊";
                ret2 = "造成12(16)點傷害，若上回合有受到生命值傷害，獲得1能量並抽1張牌";
                break;
            case 17:
                ret = "反擊架勢";
            ...
            case 34:
                ret = "以牙還牙";
                ret2 = "保留，造成9點傷害。\n這張牌在手中時，每次受到X點傷害，基礎傷害就增加(2)X，打出後重置";
                break;
            case 35:
                ret = "血償";
            ...
            case 45:
                ret = "突進";
                ret2 = "獲得3(5)點臨時護甲，從直拳與上鉤拳中選擇一張加入手牌，並給予消逝與移除";
                break;
            case 46:
                ret = "劍氣";
                ret2 = "對所有敵人造成5點傷害。獨一：將一張「納刀(+)」加入手牌，\n並給予移除。若本回合打出過「劍氣」，改為造成2次傷害";
                break;
            case 47:
                ret = "拔刀斬";
                ret2 = "造成11點傷害。力量在這張牌上有3倍效果(，臨時力量有5倍效果)。\n若本回合尚未打出過攻擊牌，獲得1點能量";
                break;
            case 48:
                ret = "模仿";
                ret2 = "丟棄1張牌，獲得3(5)點護甲，從抽牌堆檢索1張耗能為0的卡片";
                break;
            case 49:
                ret = "殘心";
                ret2 = "臨時力量與臨時敏捷在回合結束時改為減少一半的層數，而不是全部層數。移除";
                break;
            case 50:
                ret = "玄武架勢";
            ...
            case 51:
                ret = "青龍架勢";
                ret2 = "回合開始時，若上回合沒有打出過技能牌，獲得5點臨時敏捷。移除";
                break;
            case 52:
                ret = "跳步";
                ret2 = "獲得5(8)點臨時護甲，若沒有臨時敏捷，獲得2點臨時敏捷，否則獲得1點敏捷";
                break;
            case 53:
                ret = "突刺";
                ret2 = "造成7(10)點傷害，若沒有臨時力量，獲得2點臨時力量，否則獲得1點力量";
                break;
            case 54:
                ret = "棄械";
                ret2 = "丟棄1張牌，獲得4(5)點護甲與4(6)點臨時護甲";
                break;
            case 55:
                ret = "破邪";
                ret2 = "造成6點傷害，若擊殺非召喚物敵人，恢復3(5)%理智";
                break;
            case 56:
                ret = "無念";
                ret2 = "獲得10點護甲(與4點臨時護甲)，恢復7%san值，立即結束回合。移除";
                break;
            case 57:
                ret = "點燃";
                ret2 = "給予一個敵人7(10)層燃燒";
            ...
            case 58:
                ret = "納刀";
                ret2 = "從棄牌堆檢索1張消費為1的攻擊牌(，被移除時獲得1點能量)";
                break;
            case 59:
                ret = "燕返";
